			...
			for (Field fld : fields){
				...
				if (Modifier.isStatic(fld.getModifiers())){
					...
					try {
						...
						}else if ((new String[0]).getClass().equals(type)){
							p = config.get(prop.category(),fld.getName(),(String[])fld.get(null));
							fld.set(null, p.getStringList());
						}
				...
				if (Modifier.isStatic(fld.getModifiers())){
					...
					try {
						...
						if (Integer.TYPE.equals(type)){
							...
							p = config.get(prop.category(),fld.getName(),(String[])fld.get(null));
							if(prop.isSave()) config.set(prop.category(), fld.getName(), (String[])fld.get(null));
						}
						if ((null != p) && (null != comment)) {
							p.setComment(comment);
						}
